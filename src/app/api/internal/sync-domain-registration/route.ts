...
import { runDomainRegistrationBackfill } from '@/lib/services/domainRegistration.service';
...
export async function POST(request: NextRequest) {
  ...
  setTimeout(() => {
    void runDomainRegistrationBackfill(options).then((summary) => {
      console.log('[domain-registration] backfill summary', summary);
    }).catch((error) => {
      console.error('[domain-registration] backfill failed:', error);
    });
  }, 0);
  ...
  return NextResponse.json(
    {
      success: true,
      message: 'Domain registration backfill started in background.',
      options,
    },
    { status: 202 }
  );
