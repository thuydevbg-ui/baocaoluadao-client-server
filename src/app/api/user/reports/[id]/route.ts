...
export const GET = withApiObservability(async (_req: NextRequest, _ctx, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const [rows] = await db.query<any[]>(
    `SELECT id, type, target, riskScore, status, createdAt FROM user_reports WHERE id = ? AND userId = ? LIMIT 1`,
    [id, userId]
  );
  const item = rows?.[0];
...
export const DELETE = withApiObservability(async (_req: NextRequest, _ctx, { params }: { params: Promise<{ id: string }> }) => {
  const email = isMockEnabled ? 'mock@local' : await getSessionEmail(_req);
  if (!email) return NextResponse.json({ success: false, error: 'Unauthorized' }, { status: 401 });
