...
import { d1Query, isD1Configured } from '@/lib/d1Client';
import { fetchDomainExpiry } from '@/lib/scan/deterministic';

const DEFAULT_BATCH_LIMIT = Math.max(10, Number(process.env.DOMAIN_EXPIRY_BATCH || 200));
const DEFAULT_CONCURRENCY = Math.max(1, Number(process.env.DOMAIN_EXPIRY_CONCURRENCY || 4));
const DEFAULT_COOLDOWN_DAYS = Math.max(1, Number(process.env.DOMAIN_EXPIRY_COOLDOWN_DAYS || 7));
...
export interface DomainExpiryBackfillSummary {
  rowsFetched: number;
...
async function updateRows(ids: string[], expiresAt: string | null): Promise<void> {
...
export async function runDomainExpiryBackfill(options?: {
  limit?: number;
  concurrency?: number;
  cooldownDays?: number;
}): Promise<DomainExpiryBackfillSummary> {
  const db = getDb();
  ...
  const [rows] = await db.query<PendingRow[]>(
    `
      ...
      FROM scams
      WHERE type = 'website'
        AND (domain_expires_at IS NULL OR domain_expires_at = '')
        AND (domain_expires_checked_at IS NULL OR domain_expires_checked_at < DATE_SUB(NOW(), INTERVAL ? DAY))
  ...
  const worker = async () => {
    while (true) {
      ...
      try {
        ...
        const expiresAt = expiry.expiresAt ? toMysqlDateTime(expiry.expiresAt) : null;
        await updateRows(ids, expiresAt);
